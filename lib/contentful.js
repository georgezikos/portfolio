            ...
            sortedItems.map(async (item) => {
                ...
                if (isVideo && item.fields.posterImage) {
                    ...
                    try {
                        ...
                        posterBlurDataUrl = result.base64;

                        // DEBUG: Log what plaiceholder generated
                        console.log(`Video: ${item.fields.title}`);
                        console.log(
                            `  Plaiceholder dimensions: ${result.metadata.width}x${result.metadata.height}`,
                        );
                        console.log(
                            `  Aspect ratio: ${(result.metadata.width / result.metadata.height).toFixed(2)}`,
                        );
                    } catch (error) {
                        console.warn(
                ...
                return {
                    ...
                    optimizedUrl:
                        item.fields.asset?.fields?.file?.contentType?.startsWith(
                            ...
                            ? `${item.fields.asset.fields.file.url}?fm=webp&q=80&w=2000` // WebP format, 80% quality, max 2000px width
